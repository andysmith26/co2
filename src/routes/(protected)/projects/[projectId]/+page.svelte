...
<script lang="ts">
	...
	async function handleCreateTask(event: CustomEvent) {
		try {
			const { title, description, assigneeId, status } = event.detail;
			await taskStore.createTask(projectId, title, assigneeId, description);
		} catch (err) {
	...
	async function handleTaskAssignment(event: CustomEvent) {
		try {
			const { taskId, assigneeId } = event.detail;
			await taskStore.assignTask(projectId, taskId, assigneeId);
		} catch (err) {
...
<div class="container mx-auto px-4 py-8">
	...
	{:else if !project}
		<div class="text-center py-12 bg-gray-50 rounded-lg">
			<h3 class="text-lg font-medium text-gray-700 mb-2">Project Not Found</h3>
		...
		{:else}
			...
			{:else if activeTab === 'details'}
				<div class="bg-white shadow rounded-lg p-6">
					<h2 class="text-lg font-medium mb-4">Project Details</h2>
