...
<script>
	...
	function handleNewConnection(event) {
		...
		connectionsStore.update((connections) => {
			...
			localStorage.setItem(
				'ap-csp-connections',
				JSON.stringify(updatedConnections.filter((c) => c.userCreated))
			);
	...
	// Filter options
	let typeFilters = $state(nodeTypes.map((type) => type.id));
	...
	function countConnections(conceptId) {
		...
		$connectionsStore.forEach((conn) => {
			if (
				conn.source === conceptId ||
				conn.source.id === conceptId ||
				conn.target === conceptId ||
				conn.target.id === conceptId
			) {
				count++;
			}
		});
...
<main>
	<header>
		...
		<div class="search">
			<input type="text" placeholder="Search concepts..." bind:value={searchTerm} />
		</div>
	</header>

	<div class="constellation-container">
		<EnhancedConstellationGraph
			concepts={$conceptsStore.filter(
				(c) =>
					searchTerm === '' ||
		...
		<div class="sidebar" class:active={selectedConcept}>
			{#if selectedConcept}
				<div class="sidebar-header">
					<button class="close-button" onclick={() => (selectedConcept = null)}>×</button>
				</div>
					...
					{:else}
						<div class="connection-list">
							{#each getConnectedConcepts(selectedConcept.id) as item}
								<div class="connection-item" onclick={() => (selectedConcept = item.concept)}>
									...
									<div class="connection-info">
										<div class="connected-concept">
											<span
												class="concept-type-indicator"
												style="background-color: {nodeTypes.find((t) => t.id === item.concept.type)
													?.color}"
											>
											</span>
			...
			{:else}
				<div class="intro-message">
					...
					<p>
						This concept map visualizes the key ideas, vocabulary, and technologies related to
					...
					<p class="tip">
						<strong>Tip:</strong> Pay special attention to how concepts connect across different categories
						- these relationships are often tested on the AP exam!
					</p>
				</div>
			{/if}
		</div>
		...
		<div class="progress-summary">
			...
			<div class="progress-bar">
				<div
					...
					style="width: {($conceptsStore.filter((c) => $progressStore[c.id] === 'mastered').length /
						$conceptsStore.length) *
						100 || 0}%"
				></div>
				<div
					class="progress-segment in-progress"
					style="width: {($conceptsStore.filter((c) => $progressStore[c.id] === 'in-progress')
						.length /
						$conceptsStore.length) *
						100 || 0}%"
				></div>
			...
			<div class="progress-stats">
				<div class="stat">
					<span class="stat-value"
						>{$conceptsStore.filter((c) => $progressStore[c.id] === 'mastered').length}</span
					>
					<span class="stat-label">Mastered</span>
				</div>
				<div class="stat">
					<span class="stat-value"
						>{$conceptsStore.filter((c) => $progressStore[c.id] === 'in-progress').length}</span
					>
					<span class="stat-label">In Progress</span>
				</div>
				<div class="stat">
					<span class="stat-value"
						>{$conceptsStore.filter(
							(c) => !$progressStore[c.id] || $progressStore[c.id] === 'not-started'
						).length}</span
					>
				...
				<div class="stat">
					<span class="stat-value"
						>{Object.values($connectionsStore).filter((c) => c.userCreated).length}</span
					>
